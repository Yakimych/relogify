...
let getCurrentWeek = () => {
  let startDate = Js.Date.make()->DateFns.startOfWeekOpt({locale: None, weekStartsOn: Some(1)})
...
let formatDateTime = (date: Js.Date.t) => date->DateFns.format("yyyy-MM-dd HH:mm")
let formatDateTimeSeconds = (date: Js.Date.t) => date->DateFns.format("yyyy-MM-dd HH:mm:ss")
...
let toJsonDate = (date: Js.Date.t): Js.Json.t => date |> Js.Date.toISOString |> Js.Json.string

let validNumberOfGoals = (goalsString: string): int =>
  try int_of_string(goalsString) |> Js.Math.max_int(0) catch {
  | Failure(_) => 0
  }
